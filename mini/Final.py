...
msg=[]
data=[]
with open("StockSymbols.csv") as csvfile:
    symbollist = csv.DictReader(csvfile)
    for row in symbollist:
        url = "https://www.nasdaq.com/symbol/" + row['SYMBOL'] + "/after-hours"
        page = urllib.request.urlopen(url)
        soup = BeautifulSoup(page,'html.parser')
        print(row['SYMBOL'], ":")
        value = soup.find('div', {'id': 'qwidget_lastsale'}).text[1:]
...
smtp.starttls()
smtp.login('# from mail-id', '# and its password')
